...
function deleteRow($conn, $n_dm) {
    $sql = "DELETE FROM justifications WHERE n_dm = ?";
...
function searchByUserOrDistrict($conn, $search) {
    $sql = "SELECT * FROM justifications WHERE username LIKE ? OR district LIKE ?";
    $stmt = $conn->prepare($sql);
    $search = "%{$search}%";
...
?>
    ...
    <table border="1" class="table table-hover">
        <tr id="items">
            ...
            <th>Date Saisie</th>
            <th>Prix Total</th>
            <th style="color: #3df93d">Justification</th>
            <th>Action</th>
        </tr>
