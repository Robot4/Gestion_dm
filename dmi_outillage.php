...
function Searchers($conn, $search) {
...
if (isset($_POST['delete_dm']) && isset($_POST['n_dm'])) {
    ...
    Deletec($conn, $n_dm_to_delete);
}

// Check if the search form is submitted
if (isset($_POST['search_dm']) && isset($_POST['search'])) {
    $search = $_POST['search'];
    $result = Searchers($conn, $search);
} else {
...
<br>
<center>
    <h1 style="color: orange">Outillage par district</h1>
</center>
...
<table  border="1" class="table table-hover">
    ...
    <?php while ($row = $result->fetch_assoc()) { ?>
        <tr>
            ...
            <td><?php echo $row['date_verification']; ?></td>
            <td><?php echo $row['nom_verificateur']; ?></td>
