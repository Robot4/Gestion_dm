...
if (isset($conn) && $conn !== null) {
    if (isset($_POST["n_dm"])) {
        ...
        if ($result->num_rows === 1) {
            ...
            $insert_query = "INSERT INTO dmi (username, district, n_dm, n_nomenclature, designation, prix_unitaire, quantite, projet, date_saisie, prix_total, operation, quantite_maintenu, date_envoie) VALUES (
                '" . $row["username"] . "',
                '" . $row["district"] . "',
                " . $row["n_dm"] . ",
                '" . $row["n_nomenclature"] . "',
            ...
            if ($conn->query($insert_query) === true) {
                ...
                if ($conn->query($delete_query) === true) {
                    echo "Data inserted successfully into the dmi table and deleted from the dm table.";
                } else {
