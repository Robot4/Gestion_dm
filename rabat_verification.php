...
function deleteRow($conn, $n_dm) {
    $sql = "DELETE FROM magasin WHERE n_dm = ?";
...
function searchByUserOrDistrict($conn, $search) {
    $sql = "SELECT * FROM magasin WHERE username LIKE ? OR district LIKE ?";
    $stmt = $conn->prepare($sql);
    $search = "%{$search}%";
    ...
    <table border="1" class="table table-hover">
        ...
        <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
              ...
              <td style="color: #3df93d"><b><?php echo $row['operation']; ?></td></b>
